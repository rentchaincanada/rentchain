...
export const PropertyDetailPanel: React.FC<PropertyDetailPanelProps> = ({
  ...
  const readFileText = useCallback((file: File) => {
    ...
    try {
      ...
      const csvText =
        pendingCsvText ??
        (pendingFile ? await readFileText(pendingFile) : "");
      if (!csvText.trim()) {
        throw new Error("csvText required");
      }
      ...
      showToast({
        message: "Portfolio ready ƒ?" dashboard is live.",
        variant: "success",
      });
  ...
  }, [navigate, onRefresh, pendingCsvText, pendingFile, property?.id, readFileText, showToast]);const setLeasesLoadingStates = (loading: boolean, error: string | null) => {
      ...
      >
        ...
        >
          ...
          <div style={{ color: "#e5e7eb", fontWeight: 700, fontSize: "1.05rem" }}>
            {unitCount === 0 ? "â€”" : `${occupancy.toFixed(0)}%`}
          </div>
        </div>
        <div
          style={{
        ...
        >
          ...
          <div style={{ color: "#e5e7eb", fontWeight: 700, fontSize: "1.05rem" }}>
            {leaseRentRoll === 0 ? "â€”" : `${(collectionRate * 100).toFixed(0)}%`}
          </div>
        </div>
      </div>
        ...
        >
          ...
          <tbody>
            {units.length === 0 ? (
              ...
              units.map((u, idx) => {
                const unitNum = (u as any).unitNumber || "â€”";
