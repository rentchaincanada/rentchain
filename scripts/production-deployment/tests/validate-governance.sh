...
grep -Fq -- '--update-env-vars=GOOGLE_CLOUD_PROJECT=${project}' "${root}/scripts/production-deployment/prepare-human-candidate.sh"
test "$(rg -n -- '--set-env-vars|--clear-env-vars|--env-vars-file' "${root}/scripts/production-deployment/prepare-human-candidate.sh" | wc -l | tr -d ' ')" = "0"
...
grep -Fq '.candidateInstanceStarted == true' "${promotion}"
grep -Fq '.candidateSmokeHttpStatus == 200' "${promotion}"
grep -Fq '.candidateSmokeDigestVerified == true' "${promotion}"
