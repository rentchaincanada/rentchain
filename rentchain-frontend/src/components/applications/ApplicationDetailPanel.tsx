  ...
  const screeningInsights = useMemo(() => {
    ...
    if (ratio != null) {
      ...
      insights.push({
        ...
        detail:
          "Monthly income is missing or incomplete, so affordability can’t be fully assessed.",
        severity: "medium",
      });
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {application.propertyName} · Unit{" "}
          {application.unitApplied || application.unit || application.unitLabel || "—"}
          {application.coSignerName && application.coSignerName.trim() && (
            <span style={{ color: text.subtle }}>
              {" "}
              · Co-signed by {application.coSignerName}
            </span>
          )}
        </div>
        <div
      ...
      {canUseAiScreening ? (
        ...
        >
          ...
          >
            ...
            >
              Draft signals – verify before deciding
            </span>
          </div>
