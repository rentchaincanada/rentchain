...
import type { OnboardingHardeningProfile, OnboardingReference, OnboardingRestriction } from "@/api/onboardingHardeningApi";
...
function ReferenceList({ title, references }: { title: string; references: OnboardingReference[] }) {
  return (
      ...
      {references.length ? (
        references.slice(0, 12).map((reference) => (
          <Card key={reference.referenceId} style={{ borderRadius: 8, padding: 12 }}>
            ...
            {reference.redacted ? <div style={{ color: "#92400e", fontSize: 13, marginTop: 8 }}>{reference.redactionReason || "Redacted onboarding hardening reference"}</div> : null}
...
function RestrictionList({ restrictions }: { restrictions: OnboardingRestriction[] }) {
  return (
    <Section style={{ display: "grid", gap: 10 }}>
...
export function OnboardingHardeningPanel({ profile }: { profile: OnboardingHardeningProfile }) {
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <Section style={{ display: "grid", gap: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div>
            ...
            <h2 style={{ margin: "2px 0 0", fontSize: "1.15rem" }}>{label(profile.participantType)} onboarding hardening</h2>
          </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <Badge status={profile.autonomousOnboardingEnabled ? "blocked" : "verified"}>Onboarding automation disabled</Badge>
          <Badge status={profile.autonomousScreeningActivationEnabled ? "blocked" : "verified"}>Screening activation disabled</Badge>
        </div>
      </Section>

      <Section style={{ display: "grid", gap: 10 }}>
        <div style={{ fontWeight: 900 }}>Onboarding readiness summary</div>
      ...
      <RestrictionList restrictions={profile.onboardingRestrictions} />
      <ReferenceList title="Completion readiness" references={profile.completionReferences} />
      <ReferenceList title="View profile readiness" references={profile.profileReferences} />
      <ReferenceList title="View screening readiness" references={profile.screeningReadinessReferences} />
      <ReferenceList title="Integration setup readiness" references={profile.integrationReadinessReferences} />
      <ReferenceList title="Onboarding friction visibility" references={profile.frictionReferences} />
