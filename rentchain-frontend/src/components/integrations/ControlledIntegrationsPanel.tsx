...
function tone(status: string) {
  ...
  if (status === "review_required" || status === "partially_ready" || status === "partially_verified" || status === "unavailable" || status === "disabled") {
...
function ReferenceList({ title, references }: { title: string; references: ControlledIntegrationReference[] }) {
  return (
      ...
      {references.length ? (
        references.slice(0, 12).map((reference) => (
          <Card key={reference.referenceId} style={{ borderRadius: 8, padding: 12 }}>
            ...
            {reference.redacted ? <div style={{ color: "#92400e", fontSize: 13, marginTop: 8 }}>{reference.redactionReason || "Redacted controlled integration reference"}</div> : null}
...
function RestrictionList({ restrictions }: { restrictions: ControlledIntegrationRestriction[] }) {
  return (
    <Section style={{ display: "grid", gap: 10 }}>
      ...
      {restrictions.length ? (
        ...
        <Card style={{ color: "#64748b" }}>No controlled integration restrictions detected.</Card>
      )}
    </Section>
  );
}

export function ControlledIntegrationsPanel({ profile }: { profile: ControlledIntegrationProfile }) {
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <Section style={{ display: "grid", gap: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div>
            <div style={{ color: "#64748b", fontSize: 12, fontWeight: 800 }}>View integration readiness</div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <Badge status={profile.liveSynchronizationEnabled ? "blocked" : "verified"}>Live synchronization disabled</Badge>
          <Badge status={profile.autonomousExecutionEnabled ? "blocked" : "verified"}>External execution disabled</Badge>
          <Badge status={profile.webhookExecutionEnabled ? "blocked" : "verified"}>Webhook execution disabled</Badge>
        </div>
      </Section>

      <Section style={{ display: "grid", gap: 10 }}>
        <div style={{ fontWeight: 900 }}>Integration summary</div>
      ...
      <ReferenceList title="View adapter readiness" references={profile.adapterReferences} />
      ...
      <ReferenceList title="Observability readiness" references={profile.observabilityReferences} />
      <ReferenceList title="Release governance readiness" references={profile.releaseGovernanceReferences} />
