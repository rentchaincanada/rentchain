...
import { classifyPr1512Request, handlePr1512NoticesQaProxy, PR1512_QA } from "../../server/pr1512NoticesQaProxy";
...
describe("PR #1512 read-only Notices proxy", () => {
  beforeEach(() => { process.env.VERCEL_ENV = "preview"; process.env.VERCEL_GIT_COMMIT_REF = PR1512_QA.branch; });
  afterEach(() => { delete process.env.VERCEL_ENV; delete process.env.VERCEL_GIT_COMMIT_REF; vi.restoreAllMocks(); });

  it.each([
    "/api/pr1512-notices/api/landlord/notices?limit=50",
    "/api/pr1512-notices/api/landlord/notices/recipients?propertyId=pr1512-notices-fff3-property-a",
    `/api/pr1512-notices/api/landlord/notices/notice_${"a".repeat(64)}`,
    "/api/pr1512-notices/api/properties?status=active",
  ...
  it.each([
    ...
    "/api/pr1512-notices/api/landlord/notices/recipients?propertyId=foreign",
    "/api/pr1512-notices/https://production.invalid",
  ])("denies unknown or caller-selected read %s", (url) => expect(classifyPr1512Request(req(url))).toMatchObject({ allowed: false, status: 404 }));

  it.each(["POST", "PUT", "PATCH", "DELETE"])("denies %s before token acquisition", async (method) => {
    const deps = dependencies(); const res = recorder();
    await handlePr1512NoticesQaProxy(req("/api/pr1512-notices/api/landlord/notices", method), res, deps);
  ...
  it("fails closed outside the exact Preview branch", async () => {
    ...
    await handlePr1512NoticesQaProxy(req("/api/pr1512-notices/api/landlord/notices"), res, deps);
  ...
  it("uses only the fixed target and server-owned identity headers", async () => {
    const deps = dependencies(); const res = recorder();
    await handlePr1512NoticesQaProxy(req("/api/pr1512-notices/api/landlord/notices?limit=50", "GET", { authorization: "Bearer browser", "x-backend-url": "https://production.invalid", "x-rentchain-preview-qa-identity": "foreign" }), res, deps);
    expect(res.statusCode).toBe(200);
    const upstream = deps.calls.at(-1)!;
    expect(upstream.url).toBe(`${PR1512_QA.serviceUrl}/api/landlord/notices?limit=50`);
    expect(upstream.init?.headers).toMatchObject({ "x-rentchain-preview-qa-identity": PR1512_QA.selector });
  ...
  it("sanitizes WIF failure and never falls back", async () => {
    ...
    await handlePr1512NoticesQaProxy(req("/api/pr1512-notices/api/landlord/notices"), res, { fetchImpl: fetchImpl as typeof fetch, getVercelOidcToken: vi.fn(async () => "header.secret.signature") });
    expect(res.statusCode).toBe(502); expect(res.body).toEqual({ ok: false, error: "PR1512_QA_STS_FAILED" });
