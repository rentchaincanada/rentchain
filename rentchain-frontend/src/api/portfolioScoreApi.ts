...
export async function fetchPortfolioScore(portfolioId: string): Promise<{ portfolioScore: PortfolioScoreV1 }> {
  const search = new URLSearchParams();
  search.set("portfolioId", portfolioId);
  return await apiFetch<{ portfolioScore: PortfolioScoreV1 }>(`/admin/portfolio-score?${search.toString()}`);
}
