...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `evidenceRecords` | Evidence/export | Canonical source of truth for platform evidence metadata, provenance, sensitivity, and lifecycle references | Internal Firestore document ID for joins; deterministic safe `evidenceId` for audit/export references without raw ID exposure | `landlordId`, `resourceType`, `resourceId`, `evidenceClass`, `status` | Landlord-scoped evidence derivation, tenant-safe own-context projections, admin/support audit, future institutional export profiles | High to Restricted depending on evidence class | Critical | Future creation should emit or link canonical audit events where in scope; no autonomous mutation | `rentalApplications`, `screeningOrders`, `decisionActions`, `payments`, `ledgerEntries`, `workOrders`, `canonicalEvents`, domain event collections | Source of truth for evidence metadata; source collections remain operational truth | Future evidence emission services under explicit request or governed operational event | Immutable once created; supersession creates new records; retention enforcement deferred | Critical for Phase 4 institutional export readiness and evidence provenance governance |
