...
grep -Fq 'attribute_mapping["attribute.project_id"] == "assertion.project_id"' "$root_dir/checks.tf"
grep -Fq 'attribute_mapping["attribute.environment"] == "assertion.environment"' "$root_dir/checks.tf"
test "$(rg -No 'vercel_preview\.attribute_mapping == \{' "$root_dir/checks.tf" | wc -l | tr -d ' ')" = "0"
...
rg -q '^  pull-requests: read$' "$workflow_file"
rg -q '^  id-token: write$' "$workflow_file"
test "$(rg -No '^  [a-z-]+: (read|write)$' "$workflow_file" | wc -l | tr -d ' ')" = "3"
rg -q 'source_sha:' "$workflow_file"
rg -q 'pr_number:' "$workflow_file"
rg -q 'expected_head_sha:' "$workflow_file"
rg -q 'expected_base_sha:' "$workflow_file"
rg -q 'authorization_reference:' "$workflow_file"
